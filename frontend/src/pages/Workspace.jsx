    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] pt-16">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <nav className="shrink-0 lg:w-60" aria-label="Workspace settings sections">
            {/* Mobile: horizontal scroll of flat tabs */}
            <div className="overflow-x-auto pb-1 lg:hidden">
              <div className="flex gap-1 min-w-max border-b border-[var(--border-dark)] pb-px">
                {TABS.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex items-center gap-2 whitespace-nowrap rounded-lg px-3 py-2 text-xs font-medium transition-colors duration-150 ${
                        isActive
            ...
            <div className="sticky top-20 hidden flex-col gap-4 rounded-2xl border border-[var(--border-light)] bg-[var(--bg-card)] p-3 lg:flex">
              {TAB_GROUPS.map((group) => (
                <div key={group.label}>
                  <p className="px-2 pb-1.5 font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)]">
                    {group.label}
                  </p>
                    ...
                    {group.tabs.map((tab) => {
                      ...
                      return (
                        <button
                          key={tab.id}
                          onClick={() => setActiveTab(tab.id)}
                          className={`relative flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm font-medium transition-colors duration-150 ${
                            isActive
                              ? "bg-[var(--accent-soft)] text-[var(--accent)]"
          ...
          <div className="min-w-0 flex-1 space-y-6">
            {/* ===== GENERAL ===== */}
            {activeTab === "General" && (
              <div className="rounded-2xl border border-[var(--border-light)] bg-[var(--bg-card)] p-4 shadow-[var(--shadow-md)] sm:p-6">
                ...
                <div className="mt-6 space-y-6">
                  ...
                  <div className="rounded-xl border border-[var(--color-danger)]/20 bg-[var(--color-danger-soft)] p-4">
                    <h3 className="text-sm font-semibold text-[var(--color-danger)]">
                      Danger Zone
                    </h3>
                    <button
                      onClick={leaveWorkspaceHandler}
                      className="mt-2 rounded-lg border border-[var(--color-danger)]/30 bg-[var(--color-danger)]/10 px-4 py-2 text-sm font-medium text-[var(--color-danger)] transition hover:bg-[var(--color-danger)]/20"
                    >
                      Leave Workspace
                    </button>
                  </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 sm:flex-shrink-0">
                          <button
                            ...
                            className="rounded-lg border border-[var(--color-danger)]/30 bg-[var(--color-danger)]/10 p-2 text-[var(--color-danger)] transition hover:bg-[var(--color-danger)]/20"
                          >
                          ...
                          <tr key={member.userId._id} className="hover:bg-[var(--bg-hover)]/30">
                            ...
                            <td className="px-4 py-3">
                              <select
                                value={member.role}
                                onChange={(e) =>
                                  updateRoleHandler(member.userId._id, e.target.value)
                                }
                            ...
                            <td className="px-4 py-3 text-right">
                              {!isMe && (
                                <button
                                  ...
                                  className="rounded-lg border border-[var(--color-danger)]/30 bg-[var(--color-danger)]/10 px-3 py-1.5 text-sm text-[var(--color-danger)] transition hover:bg-[var(--color-danger)]/20"
                                >
                                  Remove
                                </button>
                              )}
                            ...
                            <div className="mt-3 h-48">
                              <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={analytics.dailyUsage}>
                                  ...
                                  />
                                  <Bar dataKey="tokens" fill="var(--accent-secondary)" radius={[4, 4, 0, 0]} />
                                </BarChart>
                              </ResponsiveContainer>
                            </div>
                          </div>
            ...
            {activeTab === "Branding" && (
              <div className="rounded-2xl border border-[var(--border-light)] bg-[var(--bg-card)] p-4 shadow-[var(--shadow-md)] sm:p-6">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  {workspace?.branding && (
                    <span className="w-fit rounded-full bg-[var(--color-success-soft)] px-3 py-1 text-sm font-medium text-[var(--color-success)]">
                      Active
                    </span>
                  )}
                </div>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-3 sm:gap-4">
                      ...
                      {branding.logo && (
                        <button
                          ...
                          className="text-sm text-[var(--color-danger)] hover:opacity-80"
                        >
                          Remove
                        </button>
                      )}
              ...
              <div className="rounded-2xl border border-[var(--border-light)] bg-[var(--bg-card)] p-4 shadow-[var(--shadow-md)] sm:p-6">
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[var(--text-muted)]">
                      Secret{" "}
                      {workspace?.webhookUrl ? "(leave blank to keep current)" : "(optional)"}
                    </label>
                    <input
                      type="password"
                      value={webhookSecret}
                    ...
                    <p className="mt-1 text-xs text-[var(--text-muted)]">
                      ...
                      header on every webhook. Existing secrets are never sent to the
                      browser.
                    </p>
                  </div>
                              ...
                              <div className="rounded-xl border border-[var(--border-light)] bg-[var(--bg-primary)] p-3 text-center">
                                ...
                                >
                                  {change(latest.maintainability, "maintainability") > 0
                                    ...
                                    : "↓"}{" "}
                                  {Math.abs(
                                    change(latest.maintainability, "maintainability"),
                                  )}
                                  %
                                </p>
                              </div>
