...
export default function Result({
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] px-4 py-6 text-[#f0f6fc] sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl space-y-5 relative z-10">
        ...
        <div className="flex flex-col gap-4 border-b border-[#21262d] pb-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-xl border border-[#30363d] bg-[#161b22] px-4 py-2.5">
              ...
              <div className="mt-0.5 text-2xl font-bold text-[#3fb950]">{grade}</div>
            </div>
            {onDownload && (
              <button
                onClick={onDownload}
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <ScoreCard label="Code Quality" value={scores.codeQuality} icon="◈" color="green" />
          <ScoreCard label="Security" value={scores.security} icon="◇" color="emerald" />
          <ScoreCard label="Performance" value={scores.performance} icon="↗" color="teal" />
          <ScoreCard label="Maintainability" value={scores.maintainability} icon="◎" color="green" />
        </div>
        ...
        {activeTab === "audit" && (
          <div className="space-y-4">
            <GlassCard title="Executive Summary" icon="◈">
              <p className="text-sm leading-6 text-[#8b949e]">{summary}</p>
            </GlassCard>
              ...
              <GlassCard title="Final Verdict" icon="✓">
                <div className="flex h-full min-h-[280px] items-center">
                  <div className="w-full">
                    ...
                    <p className="text-sm leading-6 text-[#8b949e]">{finalVerdict}</p>
                  </div>
                </div>
              </GlassCard>
            </div>
                  ...
                  {architecture.map((a, i) => (
                    ...
                    >
                      <div className="mb-1 flex items-center gap-2">
                        ...
                        <p className="text-xs font-semibold text-[#3fb950]">{a.component}</p>
                      </div>
                  ...
                  {securityIssues.map((s, i) => (
                    <AlertCard key={i} type="warning">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-semibold text-[#f0f6fc]">{s.issue}</p>
                          {s.risk && (
                            <p className="mt-1 text-xs text-[#f85149]">Risk: {s.risk}</p>
                          )}
                          <p className="mt-2 text-xs text-[#3fb950]">
              ...
              <GlassCard title="Generating Tests..." icon="◌">
                <div className="flex flex-col items-center gap-4 py-12">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-[#c9d1d9]">Analysing repository</p>
                    <p className="mt-1 text-xs text-[#6e7681]">Writing test cases based on your code...</p>
                  </div>
                </div>
              </GlassCard>
            )}
            ...
            {testData && !testLoading && (
              <>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge color="green">Framework: {testData.framework ?? "jest"}</Badge>
                  <Badge color="emerald">
                  ...
                  <GlassCard title="Coverage Summary" icon="◉">
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-[#8b949e]">Estimated Coverage</span>
                      ...
                      {testData.unitTests.map((fn, i) => (
                        <div key={i}>
                          <div className="mb-1 flex flex-wrap items-center gap-2">
                            ...
                            <span className="text-[10px] text-[#6e7681]">{fn.filePath}</span>
                          </div>
                          <p className="mb-3 text-[11px] text-[#6e7681]">{fn.description}</p>
                          <div className="space-y-2">
                            {fn.cases?.map((c, j) => (
                              <TestCaseRow
                                key={j}
                  ...
                  <GlassCard title="Integration Tests" icon="↗">
                    <div className="space-y-3">
                      {testData.integrationTests.map((t, i) => (
                        ...
                        >
                          <p className="text-xs font-semibold text-[#f0f6fc]">{t.label}</p>
                          <p className="mb-3 mt-1 text-[11px] text-[#6e7681]">{t.description}</p>
                          <div className="relative">
                        ...
                        <AlertCard key={i} type="warning">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-1 text-[11px] text-[#8b949e]">{m.reason}</p>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#30363d] bg-[#161b22] shadow-[0_4px_20px_rgba(0,0,0,0.12)]">
      <div className="flex items-center gap-2 border-b border-[#21262d] px-4 py-3">
        ...
        <h2 className="text-xs font-semibold text-[#c9d1d9]">{title}</h2>
      </div>
...
function ScoreCard({ label, value, icon, color = "green" }) {
  ...
  return (
    <div className="rounded-xl border border-[#30363d] bg-[#161b22] p-3.5 transition-all hover:border-[#484f58]">
      <div className="flex items-center justify-between">
        <div className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs ${colors.icon}`}>
          {icon}
        </div>
        <span className="text-[9px] uppercase tracking-wider text-[#484f58]">Score</span>
      </div>
      <div className="mt-3 flex items-end justify-between">
        <div>
          <p className="text-[10px] text-[#6e7681]">{label}</p>
          <p className={`mt-0.5 text-xl font-bold ${colors.number}`}>{val || "N/A"}</p>
        </div>
